...
- Predictive models:
  ...
  Predictive models are used to predict future values of the output variable accurately given the known information about the predictors. The goal of this models is to predict the values by making the reducible error as small as possible by improving accuracy.
...
- Define mechanistic. Define empirically-driven. How do these model types differ? How are they similar?
  ...
  - Mechanistic model: Also known as parametric methods because they make an assumption about the relationship between the independent variables and the dependent variable. In other words, these models assume a parametric form about the functional form, or shape of *f*. Even though this approach simplifies the problem of how to estimate *f*, the chosen functional form will not likely match the true form
  ...
  **Differences:**
  ...
  - Given that empirically-driven models do not make assumptions about the functional form of *f*, they required a large number of observations for the estimation. In contrast, mechanistic models only required to estimate a small number of parameters.
...
- In general, is a mechanistic or empirically-driven model easier to understand? Explain your choice.
  ...
  In general, even thought mechanistic models are less flexible, they are easier to understand than empirically-driven models. The mechanistic models normally choose to fit a simple parametric functional form that is easy to interpret. For example, linear regressions are very easy to understand and more interpretable. 
...
First, we need to load the libraries needed to produce the graphs and use the dataset
...
Finally, it is important to note that most cars have less thatn 35 miles per gallon and very few of them are above 40. Based on our knowledge of cars and their technology this should not be a surprising pattern.
...
Another interesting pattern is that both variables are round numbers which does not seem plausible in every single case. It would be interesting to take a closer at the data to understand more why is it always round. Particularly, I think thinking about prediction and statistical modeling, it is useful when data reflects more accurately the reality and it does not sound plausibly that every single car produces a round number for both `hwy` and `cty`. Probably it would also be useful to do a scatterplot that is able to show the data more accurately and account for this issue.
